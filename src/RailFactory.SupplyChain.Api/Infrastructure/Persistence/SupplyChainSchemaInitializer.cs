...
/// <summary>
/// Initializes the SupplyChain database schema for all active tenants.
/// </summary>
/// <remarks>
...
public sealed class SupplyChainSchemaInitializer(IServiceProvider serviceProvider, ILogger<SupplyChainSchemaInitializer> logger) : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogWarning("No active tenants found in catalog for SupplyChain migration.");
            return;
        }
        ...
        logger.LogInformation("SupplyChain multi-tenant schema initialization completed.");
    }
    ...
    private async Task MigrateTenantAsync(TenantResolutionResult tenant, CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogError(ex, "Failed to migrate SupplyChain database for tenant: {TenantCode}", tenant.Code);
        }
        finally
        {
