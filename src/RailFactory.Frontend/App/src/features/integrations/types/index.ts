...
export const PROVIDER_SCHEMAS: Record<string, ProviderSchema> = {
  ...
  melhorenvio: {
    providerType: 'melhorenvio',
    label: 'Melhor Envio',
    credentialFields: [
      { key: 'client_id',     label: 'Client ID',     required: true,  hint: 'ID do aplicativo no painel Melhor Envio' },
      { key: 'client_secret', label: 'Client Secret', required: true,  secret: true },
      { key: 'access_token',  label: 'Access Token',  required: true,  secret: true, hint: 'Token OAuth2 — obtenha no painel Melhor Envio após autorizar o app' },
      { key: 'refresh_token', label: 'Refresh Token', secret: true, hint: 'Opcional — usado para renovação automática' },
      { key: 'base_url',      label: 'URL Base (opcional)', placeholder: 'https://sandbox.melhorenvio.com.br', hint: 'Deixe em branco para produção. Para testes use o sandbox.' },
      { key: 'service_id',    label: 'Serviço Padrão', placeholder: '1', hint: 'ID do serviço (ex: 1=Correios PAC, 2=SEDEX). Veja documentação Melhor Envio.' },
    ],
    emitterFields: [
      { key: 'sender_name',      label: 'Nome do Remetente', required: true },
      { key: 'sender_document',  label: 'CPF / CNPJ',        required: true },
      { key: 'sender_phone',     label: 'Telefone' },
      { key: 'sender_zip_code',  label: 'CEP',               required: true },
      { key: 'sender_street',    label: 'Logradouro',         required: true },
      { key: 'sender_number',    label: 'Número',             required: true },
      { key: 'sender_complement',label: 'Complemento' },
      { key: 'sender_district',  label: 'Bairro' },
      { key: 'sender_city',      label: 'Cidade',             required: true },
      { key: 'sender_state',     label: 'UF',                 placeholder: 'SP', required: true },
    ],
    webhookFields: [
      { key: 'client_secret', label: 'Client Secret (Assinatura Webhook)', required: true, secret: true,
