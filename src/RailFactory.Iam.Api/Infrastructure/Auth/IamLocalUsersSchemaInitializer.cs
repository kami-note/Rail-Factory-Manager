    ...
    ILogger<IamLocalUsersSchemaInitializer> logger) : BackgroundService
{
    ...
    private async Task MigrateTenantAsync(TenantResolutionResult tenant, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await SeedTenantUsersAsync(dbContext, tenant.Code, cancellationToken);
    ...
    {
        await IamLocalUsersDataSeeder.SeedTenantRolesAsync(dbContext, tenantCode, logger, cancellationToken);
    }
    ...
    private async Task SeedTenantUsersAsync(
        ...
        CancellationToken cancellationToken)
    {
        await IamLocalUsersDataSeeder.SeedTenantUsersAsync(dbContext, tenantCode, environment, logger, cancellationToken);
    }
}
